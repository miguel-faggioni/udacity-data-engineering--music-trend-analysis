...
* Results
 ...
 ORDER BY year DESC;
   ...
   - avg_count_words - average number of words on the lyrics
   - avg_count_no_stopwords - average number of words on lyrics, after removing [[https://en.wikipedia.org/wiki/Stop_word][stopwords]] from it
   - avg_bpm - average number of beats per minute
   - avg_energy - average of the perceptual measure of intensity and activity. With more energetic tracks feeling fast, loud, and noisy (1.0).
   - avg_valence - average of the musical positiveness conveyed by a track. Tracks with high valence sound more positive (e.g. happy, cheerful, euphoric) (1.0), while tracks with low valence sound more negative (e.g. sad, depressed, angry) (-1.0).
   - avg_danceability - average of how suitable a track is for dancing based on a combination of musical elements including tempo, rhythm stability, beat strength, and overall regularity. A value of 0.0 is least danceable and 1.0 is most danceable.

** Top track of each year

| year | avg_count_words | avg_count_no_stopwords | avg_lyrics_sentiment | avg_bpm | avg_energy | avg_valence | avg_danceability |
|------+-----------------+------------------------+----------------------+---------+------------+-------------+------------------|
